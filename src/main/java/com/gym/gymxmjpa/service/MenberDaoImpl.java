...
import com.gym.gymxmjpa.dao.*;
import com.gym.gymxmjpa.entity.*;
...
public class MenberDaoImpl {
    ...
    public Map<String,Object> queryforerror(Map<String,Object> map1){
        //分页
        String jpal="from Member where 1=1";
        Query qu=entityManager.createQuery(jpal);
        //起始页书
    ...
    /**
     * @Description: 会员管理service实现层-根据name查询会员信息
     */
    @Transactional
    public  Member cha2(String name){
        Specification<Member> specification=new Specification<Member>() {
            ...
            public Predicate toPredicate(Root<Member> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder)  {
                Predicate predicate =criteriaBuilder.equal( root.get("memberName"),name) ;
                return predicate;
            }
        };

            Member member = menberDao.findOne(specification).orElse(null);
            if (null==member){
                Member m=new  Member();
                m.setMemberName("xx");
                return  m;
    ...
    /**
     * @Description: 会员管理service实现层-查询状态正常且余额有钱的会员信息
     */
    public List<Member>  findfornormal(){
        Specification<Member> specification=new Specification<Member>() {
            @Override
            public Predicate toPredicate(Root<Member> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder)  {
                Predicate p1 = criteriaBuilder.equal( root.get("memberStatic"), 1);
                // 构造第二个查询条件的精确匹配
                Predicate p2 =criteriaBuilder.greaterThan( root.get("memberbalance"), 0.0);
                // 将多个查询条件组合到一起
        ...
        List<Member>  list=menberDao.findAll(specification);
    ...
    /**
     * @Description: 会员管理service实现层-添加会员信息并添加充值信息
     */
    @Transactional
    public int tianjia(Member member, float ssmoney ,float zlmoney){
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        java.sql.Timestamp date=java.sql.Timestamp.valueOf(df.format(new Date()));
        member.setMemberStatic(1L);
        menberDao.save(member);
        long typeId = member.getMembertypes().getTypeId();
        Membertype membertype = memberttypeDao.findById(typeId).get();
       float price=membertype.getTypemoney();
        Chongzhi  cz=new Chongzhi();
        cz.setMoney((long) price);
        cz.setSsmoney((long) ssmoney);
        cz.setZlmoney( (long) zlmoney);
        cz.setCzStatic(2l);
        cz.setBeizhu("");
        cz.setDate(date);
        cz.setMember(member);
        cz.setMembertype(membertype);
        chongZhiDao.save(cz);
        return 1;
    }
